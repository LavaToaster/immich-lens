        ...
        Group {
            if isLoading && assets.isEmpty {
                ...
                ContentUnavailableView(
                    ...
                    description: Text("No photos found for \(person.name)")
                )
            } else {
                AssetGridView(
                    assets: assets,
    ...
    private func loadAssets() async {
        ...
        do {
            let response = try await client.getTimeBuckets(
                query: .init(personId: person.id, visibility: .timeline))

            let bucketsResponse = try response.ok.body.json
            logger.info("Loaded \(bucketsResponse.count) time buckets for person \(person.name)")

            guard !bucketsResponse.isEmpty else {
                logger.info("No time buckets for person \(person.name)")
                return
            }
            ...
            logger.error("Failed to fetch assets for person \(person.name): \(error.localizedDescription)")
        }
    }
}
